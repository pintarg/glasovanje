...
io.sockets.on("connection", function(socket) {
  ...
  socket.on("socketPisanjeOdg", function(msg) {
    ...
    if (msg.SocketID == "webpage") {
      ...
      clientRedis.hget("stanjeodg", cIP1, function(err, reply) {
        if (reply === null) {
          clientRedis.hset("stanjeodg", cIP1, '{"hms":"'+hms+'","stanjeOdg":"1"}');
        } else {
          tempReply=JSON.parse(reply);
          clientRedis.hset("stanjeodg", cIP1, '{"hms":"'+hms+'","stanjeOdg":"'+(++tempReply.stanjeOdg)+'"}');
        }
      });
    } else {
