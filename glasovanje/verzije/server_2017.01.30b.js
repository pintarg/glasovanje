...
var VprID, // ID vprašanja v bazi "vprasanja"
    ...
    tempHkeysWebGE; // spremenljivka za posamezen key 'webge' Redis tabele
...
io.sockets.on("connection", function(socket) {
  ...
  socket.on("socketPisanjeOdg", function(msg) {
    ...
    if (msg.SocketID == "webpage") {
      SocketID = socket.id;
      cIP1=socket.request.connection.remoteAddress.substring(7); // substring odreže prvih 7 znakov (":ffff::")
      clientRedis.hget("webge", cIP1, function(err, reply) {
        tempReply=JSON.parse(reply);
        clientRedis.hset("webge", cIP1, '{"IP":"'+tempReply.IP+'","SocketID":"'+tempReply.SocketID+'","hms":"'+hms+'","stanjeOdg":"'+(++tempReply.stanjeOdg)+'"}');
      });
    } else {
  ...
  // function WebGEQRst() { // funkcija za reset vrednosti 'stanjeOdg' ob branju drugega vprašanja
  //   var k=0;
  //   for(i=0; i<hkeysWebGE.length; i++) {
  //     tempHkeysWebGE=hkeysWebGE[k];
  //     clientRedis.hget("webge", tempHkeysWebGE, function(err, reply) {
  //       console.log((k+1)+". zapis: "+hkeysWebGE[k]);
  //       tempReply=JSON.parse(reply);
  //       console.log("Nov Q, stara vrednost: "+'{"IP":"'+tempReply.IP+'","SocketID":"'+tempReply.SocketID+'","hms":"'+tempReply.hms+'","stanjeOdg":"'+tempReply.stanjeOdg+'"}');
  //       clientRedis.hset("webge", tempHkeysWebGE, '{"IP":"'+tempReply.IP+'","SocketID":"'+tempReply.SocketID+'","hms":"99:99:99","stanjeOdg":"0"}')
  //       console.log("Nov Q, nova vrednost: "+'{"IP":"'+tempReply.IP+'","SocketID":"'+tempReply.SocketID+'","hms":"99:99:99","stanjeOdg":"0"}');
  //       k++;
  //     });
  //   }
  // }
  ...
  // var currentKey, stanjeWebGE=[];
  // function WebGETabela() {
  //   console.log("WebGETabela-F");
  //   for(i=0; i<hkeysWebGE.length; i++) {
  //     console.log("WebGETabela-Loop: "+i);
  //     currentKey=hkeysWebGE[i];
  //     clientRedis.hget("webge1", currentKey, function(err, reply) {
  //       tempReply=JSON.parse(reply);
  //       console.log("WebGETabela-DB: IP:"+tempReply.IP+", SocketID:"+tempReply.SocketID+", stanjeOdg: Ob "+tempReply.hms+" "+tempReply.stanjeOdg+". prejem odgovora na ?. vprašanje.");
  //       // stanjeWebGE[i]={"IP":tempReply.IP,"SocketID":tempReply.SocketID,"stanjeOdg":"Ob "tempReply.hms" "tempReply.stanjeOdg". prejem odgovora na ?. vprašanje."};
  //       stanjeWebGE[i]={"IP":tempReply.IP,"SocketID":tempReply.SocketID,"hms":tempReply.hms,"stanjeOdg":tempReply.stanjeOdg};
  //     });
